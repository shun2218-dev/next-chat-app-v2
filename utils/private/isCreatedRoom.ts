...
const registerRoom = async (uid: string, partnerid: string, roomid: string) => {
  const partnerRef = doc(db, 'users', partnerid, 'rooms', uid);
  await setDoc(partnerRef, { roomid });
};

const createRoom = async (id: string, uid: string, partnerid: string, message: string) => {
  await addDoc(collection(db, 'rooms'), { owner: uid, partner: partnerid })
    .then(async (res) => {
      await registerRoom(uid, partnerid, res.id);
      await registerRoom(partnerid, uid, res.id);
      id = res.id;
      return res.id;
    })
    .then(async (roomid) => {
...
const isCreatedRoom = async (uid: string, partnerid: string, message: string) => {
